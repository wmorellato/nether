...
serverSchema.pre('save', function(next) {
  ...
  if (server.isModified('sshKey')) {
    server.sshKey = CryptoJS.AES
        .encrypt(server.sshKey, process.env.SSHKEY_ENCRYPTION_KEY);
  }
...
serverSchema.methods.getSshKey = function() {
  ...
  const decSshKey = CryptoJS.AES
      .decrypt(server.sshKey, process.env.SSHKEY_ENCRYPTION_KEY);
