 ...
 */
const manageInstance = async (instanceId, action) => {
    var response = undefined
    var pendingInstances = undefined
        ...
        try {
            ...
            pendingInstances.forEach((instance) => {
                if (instance.InstanceId === instanceId) {
                    resolve({ ec2InstanceId: instance.InstanceId, state: instance.CurrentState.Name })
                }
            })
        } catch (e) {
            debug(e)
 ...
 */
const getInstancesInfo = async (instancesIds) => {
    const response = []
